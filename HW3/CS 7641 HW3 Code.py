...
# 
# Datasets: Titanic Data, Bank Marketing.
# 
...
os.chdir(r"/Users/kaiwei/Desktop/GATECH/ML/hw3/CS-7641---Machine-Learning-master/Assignment 3 Unsupervised Learning") 


# ### Load the Titanic Pre-Precessing Data

# In[57]:


## Download the Titanic Data from kaggle
df_tt = pd.read_csv('ttData_preprocessed.csv')
print("Data has",len(df_tt),"rows and", len(df_tt.columns),"columns.")
if df_tt.isnull().values.any():
...
def import_data():
    ...
    X1 = np.array(df_tt.values[:,1:-1],dtype='int64')
    Y1 = np.array(df_tt.values[:,0],dtype='int64')
...
from sklearn.metrics import silhouette_score as sil_score, f1_score, homogeneity_score, mean_squared_error
import matplotlib.pyplot as plt
...
def run_kmeans(X,y,title):
    ...
    sil_scores = []; f1_scores = []; homo_scores = []; train_times = [];SSE = [];
...
ttX,ttY,bankX,bankY = import_data()
run_kmeans(bankX,bankY,'Bank Data')
...
# This section will implement 4 different dimensionality reduction techniques on both the titanic and the banking dataset. Then, k-means and EM clustering will be performed for each (dataset * dim_reduction) combination to see how the clustering compares with using the full datasets. The 4 dimensionality reduction techniques are:
...
def run_PCA(X,y,title):
    ...
    pca = PCA(n_components = 10, random_state=5).fit(X) #for all components
...
ttX,ttY,bankX,bankY = import_data()
run_PCA(ttX,ttY,"Titanic Data")
run_ICA(ttX,ttY,"Titanic Data")
run_RCA(ttX,ttY,"Titanic Data")
imp_titanic, topcols_titanic = run_RFC(ttX,ttY,df_tt)
...
# Recreating Clustering Experiment (k-means and EM) for titanic data.
...
print('rfc')
run_kmeans(rfc_tt,ttY,'RFC titanic Data')


# In[70]:


evaluate_kmeans(KMeans(n_clusters=14,n_init=10,random_state=100,n_jobs=-1),pca_tt,ttY)
evaluate_kmeans(KMeans(n_clusters=12,n_init=10,random_state=100,n_jobs=-1),ica_tt,ttY)
evaluate_kmeans(KMeans(n_clusters=10,n_init=10,random_state=100,n_jobs=-1),rca_tt,ttY)
evaluate_kmeans(KMeans(n_clusters=14,n_init=10,random_state=100,n_jobs=-1),rfc_tt,ttY)


# In[24]:


run_EM(pca_tt,ttY,'PCA tt Data')
run_EM(ica_tt,ttY,'ICA tt Data')
run_EM(rca_tt,ttY,'RCA tt Data')
run_EM(rfc_tt,ttY,'RFC tt Data')


# In[71]:


evaluate_EM(EM(n_components=18,covariance_type='diag',n_init=1,warm_start=True,random_state=100),pca_tt,ttY)
evaluate_EM(EM(n_components=28,covariance_type='diag',n_init=1,warm_start=True,random_state=100),ica_tt,ttY)
evaluate_EM(EM(n_components=25,covariance_type='diag',n_init=1,warm_start=True,random_state=100),rca_tt,ttY)
evaluate_EM(EM(n_components=22,covariance_type='diag',n_init=1,warm_start=True,random_state=100),rfc_tt,ttY)
...
# This section will train a neural network on the 4 projected datasets for only the tt data. We will examine the learning curves on the training data as well as the final network performance on the test dataset.

# In[27]:


# Original, full dataset
X_train, X_test, y_train, y_test = train_test_split(np.array(ttX),np.array(ttY), test_size=0.20)
full_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_full, NN_train_score_full, NN_fit_time_full, NN_pred_time_full = plot_learning_curve(full_est, X_train, y_train,title="Neural Net Titanic: Full")
...
X_train, X_test, y_train, y_test = train_test_split(np.array(pca_tt),np.array(ttY), test_size=0.20)
pca_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_pca, NN_train_score_pca, NN_fit_time_pca, NN_pred_time_pca = plot_learning_curve(pca_est, X_train, y_train,title="Neural Net Titanic: PCA")
...
X_train, X_test, y_train, y_test = train_test_split(np.array(ica_tt),np.array(ttY), test_size=0.20)
ica_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_ica, NN_train_score_ica, NN_fit_time_ica, NN_pred_time_ica = plot_learning_curve(ica_est, X_train, y_train,title="Neural Net tt: ICA")
...
X_train, X_test, y_train, y_test = train_test_split(np.array(rca_tt),np.array(ttY), test_size=0.20)
rca_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_rca, NN_train_score_rca, NN_fit_time_rca, NN_pred_time_rca = plot_learning_curve(rca_est, X_train, y_train,title="Neural Net tt: RCA")
...
X_train, X_test, y_train, y_test = train_test_split(np.array(rfc_tt),np.array(ttY), test_size=0.20)
rfc_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_rfc, NN_train_score_rfc, NN_fit_time_rfc, NN_pred_time_rfc = plot_learning_curve(rfc_est, X_train, y_train,title="Neural Net tt: RFC")
...
compare_fit_time(train_samp_full, NN_fit_time_full, NN_fit_time_pca, NN_fit_time_ica, 
                 NN_fit_time_rca, NN_fit_time_rfc, 'tt Dataset')              
compare_pred_time(train_samp_full, NN_pred_time_full, NN_pred_time_pca, NN_pred_time_ica, 
                 NN_pred_time_rca, NN_pred_time_rfc, 'tt Dataset')   
compare_learn_time(train_samp_full, NN_train_score_full, NN_train_score_pca, NN_train_score_ica, 
                 NN_train_score_rca, NN_train_score_rfc, 'tt Dataset')  


# # 7. Training Neural Network on Projected Data with Cluster Labels

# This section will train a neural network on the 4 projected datasets for only the tt data. The difference in this section is that we now add cluster labels from both k-means and EM (after 1-hot encoding) to the reduced datasets. We will examine the learning curves on the training data as well as the final network performance on the test dataset.
...
km = KMeans(n_clusters=9,n_init=10,random_state=100,n_jobs=-1).fit(ttX)
km_labels = km.labels_
em = EM(n_components=24,covariance_type='diag',n_init=1,warm_start=True,random_state=100).fit(ttX)
em_labels = em.predict(ttX)

clust_full = addclusters(ttX,km_labels,em_labels)
clust_pca = addclusters(pca_tt,km_labels,em_labels)
clust_ica = addclusters(ica_tt,km_labels,em_labels)
clust_rca = addclusters(rca_tt,km_labels,em_labels)
clust_rfc = addclusters(rfc_tt,km_labels,em_labels)
...
# Original, full dataset
X_train, X_test, y_train, y_test = train_test_split(np.array(clust_full),np.array(ttY), test_size=0.20)
full_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_full, NN_train_score_full, NN_fit_time_full, NN_pred_time_full = plot_learning_curve(full_est, X_train, y_train,title="Neural Net Titanic with Clusters: Full")
final_classifier_evaluation(full_est, X_train, X_test, y_train, y_test)
...
X_train, X_test, y_train, y_test = train_test_split(np.array(clust_pca),np.array(ttY), test_size=0.20)
pca_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_pca, NN_train_score_pca, NN_fit_time_pca, NN_pred_time_pca = plot_learning_curve(pca_est, X_train, y_train,title="Neural Net Titanic with Clusters: PCA")
final_classifier_evaluation(pca_est, X_train, X_test, y_train, y_test)
...
X_train, X_test, y_train, y_test = train_test_split(np.array(clust_ica),np.array(ttY), test_size=0.20)
ica_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_ica, NN_train_score_ica, NN_fit_time_ica, NN_pred_time_ica = plot_learning_curve(ica_est, X_train, y_train,title="Neural Net tt with Clusters: ICA")
final_classifier_evaluation(ica_est, X_train, X_test, y_train, y_test)
...
X_train, X_test, y_train, y_test = train_test_split(np.array(clust_rca),np.array(ttY), test_size=0.20)
rca_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_rca, NN_train_score_rca, NN_fit_time_rca, NN_pred_time_rca = plot_learning_curve(rca_est, X_train, y_train,title="Neural Net tt with Clusters: RCA")
final_classifier_evaluation(rca_est, X_train, X_test, y_train, y_test)
...
X_train, X_test, y_train, y_test = train_test_split(np.array(clust_rfc),np.array(ttY), test_size=0.20)
rfc_est = MLPClassifier(hidden_layer_sizes=(50,), solver='adam', activation='logistic', learning_rate_init=0.01, random_state=100)
train_samp_rfc, NN_train_score_rfc, NN_fit_time_rfc, NN_pred_time_rfc = plot_learning_curve(rfc_est, X_train, y_train,title="Neural Net tt with Clusters: RFC")
final_classifier_evaluation(rfc_est, X_train, X_test, y_train, y_test)
